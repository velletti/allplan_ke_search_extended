...
class AllplanDocumentationsIndexer extends BaseKeSearchIndexerHook
{
    /**
    ...
    public function main(&$indexerConfig, &$indexerObject) {
        ...
        $queryBuilder->select('*')->from('tx_maritelearning_domain_model_download');
     ...
     * @return string
     */
    public function getFileContent( ConnectionPool $connectionPool , $uid) {

        /** @var QueryBuilder $queryBuilder */
        $queryBuilder = $connectionPool->getConnectionForTable('sys_file')->createQueryBuilder();
        $queryBuilder->select('sf.uid', 'sf.missing','sf.identifier','sf.name','sf.sha1','sf.creation_date','sf.modification_date')
            ->from('sys_file' , 'sf')
            ->leftJoin('sf' , "sys_file_reference" , "sfr" , "sf.uid = sfr.uid_local") ;

        $expr = $queryBuilder->expr();
        $queryBuilder->where(
            $expr->eq('sf.missing', 0 )
        )->andWhere(
            $expr->eq('sfr.fieldname', $queryBuilder->createNamedParameter( 'tx_maritelearning_domain_model_download_download' , Connection::PARAM_STR ))
        )->andWhere(
            $expr->eq('sfr.uid_foreign', $queryBuilder->createNamedParameter( $uid , Connection::PARAM_INT ))
        ) ;
